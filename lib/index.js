...
module.exports = new class {
    ...
    capturePromise(output, writer, context, config) {
        ...
        output
            ...
            .then(result => context[`_${index}`] = result)
    ...
    filterHistory(lines, maxLength) {
        let last = null
        lines = lines.filter(l => l.trim() && l !== last && (last = l))
    ...
    loadHistory(server, path, maxLength) {
        ...
        const history = this.filterHistory(lines, maxLength)
        history.reverse().forEach(l => server.history.push(l))
    }

    saveHistory(server, path, maxLength) {
        const history = server.history.slice().reverse()
        const lines = this.filterHistory(history, maxLength)
    ...
    registerCommand(server, config) {
        ...
        const history = () => {
            const data = server.history.slice().reverse().join('\n')
            this.print(data, defaultGray, config.prompt)
        }
        ...
        const modules = () => {
            const text = util.inspect(config.module || {})
