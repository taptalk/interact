...
module.exports = new class {
    ...
    loadConfig(context, config, reload) {
        ...
        if (config.preLoad) {
            try {
                config.preLoad(context, temp)
            } catch (e) { console.error(e) || reload || process.exit() }
        }
        if (reload) {
        ...
        if (config.postLoad) {
            try {
                config.postLoad(context, temp)
            } catch (e) { console.error(e) || reload || process.exit() }
        }
        if (!reload) {
            if (config.postInit) {
                try {
                    config.postInit(context)
    ...
    registerCommand(server, config) {
        ...
        const reload = () => {
            ...
            this.print('reloaded ', defaultGray, config.prompt)
        }
    ...
    setup(config) {
        ...
        this.put(`         \r${config.prompt || defaultPrompt}`)
